...
@section('title', 'Ubah item inventory')

@section('content')
    <form action="{{ route('plantation.inventory-items.update', [$entity, $item]) }}" method="POST" class="max-w-xl space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
